...
/*!
 * PhotoSphereViewer.VirtualTourPlugin 5.8.2
...
"use strict";
  ...
  function gpsDegToRad(gps) {
    return [import_three3.MathUtils.degToRad(gps[0]), import_three3.MathUtils.degToRad(gps[1])];
  }
  ...
  function checkArrowStyle(style) {
    if (style && ["color", "hoverColor", "outlineColor"].some((key) => key in style)) {
      import_core2.utils.logWarn(`VirtualTourPlugin: "arrowStyle" does not support color, hoverColor, outlineColor anymore`);
      return void 0;
    }
    if (style && typeof style.size === "number") {
      import_core2.utils.logWarn(`VirtualTourPlugin: "arrowStyle.size" must be an object (width + height, in pixels)`);
      delete style.size;
    }
    return style;
  }
  ...
  var ArrowsRenderer = class extends import_core3.AbstractComponent {
    constructor(parent, plugin) {
      ...
      this.camera = this.is3D ? new import_three4.PerspectiveCamera(30, 1) : null;
      this.scene = new import_three4.Scene();
      this.viewer.addEventListener(import_core3.events.ReadyEvent.type, this, { once: true });
      this.viewer.addEventListener(import_core3.events.PositionUpdatedEvent.type, this);
      this.viewer.addEventListener(import_core3.events.SizeUpdatedEvent.type, this);
      this.viewer.addEventListener(import_core3.events.RenderEvent.type, this);
    ...
    destroy() {
      this.viewer.removeEventListener(import_core3.events.ReadyEvent.type, this);
      this.viewer.removeEventListener(import_core3.events.PositionUpdatedEvent.type, this);
      this.viewer.removeEventListener(import_core3.events.SizeUpdatedEvent.type, this);
      this.viewer.removeEventListener(import_core3.events.RenderEvent.type, this);
      this.viewer.removeEventListener(import_core3.events.ClickEvent.type, this);
      super.destroy();
    }
    handleEvent(e) {
      switch (e.type) {
        case import_core3.events.ReadyEvent.type:
        case import_core3.events.SizeUpdatedEvent.type:
        case import_core3.events.PositionUpdatedEvent.type:
          this.__updateCamera();
          break;
        case import_core3.events.RenderEvent.type:
          this.render();
          break;
        case import_core3.events.ClickEvent.type: {
          const link = this.getTargetLink(e.data.target, true);
          if (link) {
        ...
        case "mouseenter":
          {
            const link = this.getTargetLink(e.target);
            if (link) {
        ...
        case "mousemove":
          {
            const link = this.getTargetLink(e.target, true);
            if (link) {
    ...
    __updateCamera() {
      ...
      if (this.is3D) {
        ...
        position.pitch = import_three4.MathUtils.clamp(position.pitch, -this.arrowsPosition.maxPitch, -this.arrowsPosition.minPitch);
    ...
    render() {
      if (this.is3D) {
        ...
        this.scene.children.forEach((object) => {
          ...
          if (data.conflict) {
            const distance2 = Math.abs(import_core3.utils.getShortestArc(position.yaw, data.yaw));
    ...
    addLinkArrow(link, position, depth = 1) {
      ...
      if (conf.className) {
        import_core3.utils.addClasses(element, conf.className);
      }
      if (conf.style) {
      ...
      if (this.is3D) {
        ...
        this.scene.children.forEach((object2) => {
          ...
          if (Math.abs(import_core3.utils.getShortestArc(data.yaw, position.yaw)) < this.arrowsPosition.linkOverlapAngle) {
            data.conflict = true;
            conflict = true;
          }
        });
    ...
    getTargetLink(target, closest = false) {
      const target2 = closest ? import_core3.utils.getClosest(target, ".psv-virtual-tour-link") : target;
    ...
    },
    {
      ...
      renderMode(renderMode) {
        if (renderMode === "markers") {
          import_core7.utils.logWarn(`VirtualTourPlugin: "renderMode" markers has been replaced by 2d`);
          return "2d";
        }
      ...
      markerStyle(markerStyle) {
        if (markerStyle) {
          import_core7.utils.logWarn(`VirtualTourPlugin: "markerStyle" is deprecated`);
        }
        return null;
      },
      arrowPosition(arrowPosition) {
        if (arrowPosition) {
          import_core7.utils.logWarn(`VirtualTourPlugin: "arrowPosition" is deprecated`);
        }
        return null;
      },
      arrowsPosition(arrowsPosition, { defValue, rawConfig }) {
        if (!import_core7.utils.isNil(rawConfig.markerPitchOffset)) {
          import_core7.utils.logWarn(`VirtualTourPlugin: "markerPitchOffset" is deprecated, use "arrowsPosition.linkPitchOffset" instead`);
  ...
  VirtualTourPlugin.VERSION = "5.8.2";
