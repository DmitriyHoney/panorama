...
/*!
 * PhotoSphereViewer.MarkersPlugin 5.8.2
...
"use strict";
  ...
  var import_core17 = require_core();
...
`;
  ...
  var import_core16 = require_core();
  ...
  var Marker = class {
    ...
    update(config) {
      ...
      if (import_core7.utils.isExtendedPosition(config)) {
        import_core7.utils.logWarn('Use the "position" property to configure the position of a marker');
        config.position = this.viewer.dataHelper.cleanPosition(config);
      }
      if ("width" in config && "height" in config) {
        import_core7.utils.logWarn('Use the "size" property to configure the size of a marker');
    ...
    update(config) {
      ...
      if (import_core11.utils.isExtendedPosition(this.config.position)) {
        ...
        if (this.config.orientation) {
          import_core11.utils.logWarn(`Marker#orientation is deprecated, use "rotation.yaw" or "rotation.pitch" instead`);
    ...
    update(config) {
      ...
      try {
        ...
        if (this.isPixels) {
          ...
          this.definition = actualPoly.map((coords) => {
            return coords.map((coord) => {
              return [import_core14.utils.parseAngle(coord[0]), import_core14.utils.parseAngle(coord[1], true)];
            });
          });
        }
      } catch (e) {
  ...
  var MarkersPlugin = class extends import_core16.AbstractConfigurablePlugin {
    ...
    showMarkersList() {
      ...
      this.viewer.panel.show({
        ...
        noMargin: true,
        clickHandler: (target) => {
          const li = import_core16.utils.getClosest(target, "li");
